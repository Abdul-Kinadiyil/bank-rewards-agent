...
apigeecli apis create bundle -n adk-bank-rewards-v1 \
...
apigeecli products create --name "adk-bank-rewards-product" --display-name "adk-bank-rewards-product" \
  --opgrp ./config/adk-bank-rewards-product-ops.json --envs "$APIGEE_ENV" \
  --approval auto --org "$PROJECT_ID" --token "$TOKEN"

echo "Creating Developer"
apigeecli developers create --user adk-bank-rewards-developer \
  --email "adk-bank-rewards-developer@acme.com" --first="ADK Bank Rewards" \
  --last="Sample User" --org "$PROJECT_ID" --token "$TOKEN"

echo "Creating Developer App"
apigeecli apps create --name adk-bank-rewards-app --email "adk-bank-rewards-developer@acme.com" \
  --prods "adk-bank-rewards-product" --org "$PROJECT_ID" --token "$TOKEN" --disable-check

APIKEY=$(apigeecli apps get --name "adk-bank-rewards-app" --org "$PROJECT_ID" --token "$TOKEN" --disable-check | jq ."[0].credentials[0].consumerKey" -r)

SECRET_ID="cymbal-bank-rewards-apikey"
...
export APIKEY
export PROXY_URL="$APIGEE_HOST/v1/crm"
...
echo "Rewards: "
echo " "
echo "curl --location \"https://$APIGEE_HOST/v1/crm/rewards\" \
--header \"Content-Type: application/json\" \
--header \"x-apikey: $APIKEY\""
echo " "
echo "Members: "
echo " "
echo "curl --location \"https://$APIGEE_HOS/v1/crm/members\" \
--header \"Content-Type: application/json\" \
--header \"x-apikey: $APIKEY\""
echo " "
